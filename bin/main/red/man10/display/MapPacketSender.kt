            ...
            for (player in players) {
                ...
                for (packet in packets) {
                    try {
                        ...
                        errors++
                        safeLog("ERROR", "Packet send failed to ${player.name}: ${ex.javaClass.simpleName}")
             ...
             for (index in 0 until modifier.size()) {
                 ...
                 if (!wrotePatch && (matchesType(innerType, patchClass) || (current != null && patchClass.isInstance(current)))) {
                     try {
                         ...
                         safeLog("INFO", "MapPatch written to Optional field[$index]")
                         wrotePatch = true
                         continue
                     } catch (e: Exception) {
